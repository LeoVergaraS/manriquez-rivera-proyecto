...
import { Card, Badge, Table, Container, Col, Row } from "react-bootstrap";
...
const Clientes = ({
  ...
  const getSesionesByIdCaso = async (id) => {
    try {
      ...
      if (dropSiempre == 1) {
        ...
        let url =
          "http://localhost:8090/consultas/sesiones/id_caso/" +
          id +
          "/" +
          fechaInicio +
          "/" +
  ...
  const createCasoOption = (caso) => {
    return {
      label: `${caso.id_cliente.nombre} - ${
        caso.id_materia.nombre
