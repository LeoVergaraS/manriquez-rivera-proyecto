...
const Materias = ({id_abogado,fechaInicio,fechaFin,dropSelect,dropSiempre,dropAnio,setDropSelect,}) => {
	...
	const getSesionesByMateriaDesdeSiempre = async (id_abogado) => {
		try {
			...
			let url =
				"http://localhost:8090/consultas/materia/sesiones/" + materia.id + "/" +id_abogado;
			const response = await axios.get(url,config);
			if (response.status === 200) {
	...
	const getSesionesByMateria = async (id_abogado,fechaInicio,fechaFin,dropSelect,dropSiempre,dropAnio) => {
		try {
			...
			if (dropSiempre == 1) {
				...
				let url ="http://localhost:8090/consultas/materia/sesiones/" + materia.id + "/" + fechaInicio + "/" + fechaFin + "/" + dropSelect + "/" +
					id_abogado;
				const response = await axios.get(url,config);
				if (response.status === 200) {
	...
	const getEstadisticas = async (id_abogado,fechaInicio,fechaFin,dropSiempre) => {
		try {
			...
			let url ="http://localhost:8090/consultas/materia/estadisticas/" + id_abogado + "/" + materia.id + "/" + fechaInicio + "/" + fechaFin +
				"/" +
				dropSiempre;
			const response = await axios.get(url,config);
			if (response.status === 200) {
